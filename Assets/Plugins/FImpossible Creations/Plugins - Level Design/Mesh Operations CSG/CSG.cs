...
namespace Parabox.CSG
{
    ...
    public static class CSG
    {
        ...
        /// <param name="rhs">The input mesh of the boolean operation.</param>
        /// <returns>A new mesh if the operation succeeds, or null if an error occurs.</returns>
        public static Model Subtract(Mesh s, Material sm, Matrix4x4 smx, Mesh o, Material om, Matrix4x4 omx, bool allPoly, bool flipB = false)
        {
            Model csg_model_a = new Model(s, sm, smx);
            Model csg_model_b = new Model(o, om, omx);
            ...
            if ( allPoly)
                polygons = Node.Subtract(a, b, flipB).AllPolygons();
            else
                polygons = Node.Subtract(a, b, flipB).ClipPolygons(a.polygons);
