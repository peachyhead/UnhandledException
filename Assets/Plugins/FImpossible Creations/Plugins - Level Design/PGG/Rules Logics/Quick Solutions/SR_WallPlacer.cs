...
namespace FIMSpace.Generating.Rules.QuickSolutions
{
    public class SR_WallPlacer : SpawnRuleBase, ISpawnProcedureType
    {
        ...
        [HideInInspector] public ECornerMode CornerMode = ECornerMode.In;
        [HideInInspector] public ECornerMode45 CornerMode45 = ECornerMode45.Lined;

        [HideInInspector] private ESR_Space SpawnOn = ESR_Space.OutOfGrid;
        [HideInInspector] private string OccupiedTags = "";
        ...
        public enum EOverlapCollisionMode { Simple, BoundsCollision }
        [HideInInspector] public EOverlapCollisionMode OverlapDetectMode = EOverlapCollisionMode.Simple;

        [HideInInspector]
        [Tooltip("Other way algorithm is removing overlaps")] public bool Version2 = false;
        [HideInInspector][Range(0f, 1.5f)] public float ScaleBounds = 0.99f;
        ...
        public override void NodeHeader()
        {
            ...
            //{
            //    /*if (Module != EWallModule.Curve90) */
            //    GUIIgnore[0] = "SpawnOnEachSide";
            //    //else GUIIgnore[0] = "";
        ...
        {
            if (sp == null) sp = so.FindProperty("CornerMode");
            var spc = sp.Copy();
            ...
            if (Module == EWallModule.WallBase)
            {
                ...
                spc.Next(false); EditorGUILayout.PropertyField(spc);
                spc.Next(false); spc.Next(false); spc.Next(false); 
                spc.Next(false); spc.Next(false); spc.Next(false);
                spc.Next(false); EditorGUILayout.PropertyField(spc);
                spc.Next(false); EditorGUILayout.PropertyField(spc);
            }
            else if (Module == EWallModule.Curve90)
            {
                ...
                spc.Next(false); GUILayout.Space(9);

                if (OverlapDetectMode == EOverlapCollisionMode.Simple)
                {
                    EditorGUIUtility.labelWidth = 30;
                    if (CornerMode == ECornerMode.In) EditorGUILayout.PropertyField(spc, new GUIContent("V2:", spc.tooltip));
                    spc.Next(false);
                }
                else
                { EditorGUIUtility.labelWidth = 80; spc.Next(false); EditorGUILayout.PropertyField(spc); }

                EditorGUILayout.EndHorizontal(); EditorGUIUtility.labelWidth = 0;

                spc.Next(false);
                spc.Next(false); EditorGUILayout.PropertyField(spc);
                spc.Next(false); EditorGUILayout.PropertyField(spc);
                spc.Next(false); spc.Next(false); spc.Next(false); EditorGUILayout.PropertyField(spc);
                ...
                else
                {
                    ...
                    for (int r = 0; r < 4; r++)
                    {
                        ...
                        if (AutoRemoveOverlaps)
                        {
                            ...
                            {
                                var spawnsInCell = cell.GetSpawnsJustInsideCell();
                                Bounds myBounds = GetSpawnBounds(spawn, false);


                                Quaternion rot = Quaternion.LookRotation(toCell) * Quaternion.Euler(0, YawOffset, 0);
                                myBounds = FEngineering.TransformBounding(myBounds, Matrix4x4.TRS(rot * GetUnitOffset(DirectOffset, OffsetMode, preset), rot, Vector3.one));

                                myBounds.size *= ScaleBounds;
                                for (int i = 0; i < spawnsInCell.Count; i++)
                                {
                                    if (spawnsInCell[i].OwnerMod != mod) continue;
                                    Bounds oBounds = GetSpawnBounds(spawnsInCell[i]);
                                    oBounds.size *= ScaleBounds;

                                    if (myBounds.Intersects(oBounds)) { ScheduleToRemove(spawnsInCell[i]); }
                                }

                                myBounds.center += preset.GetCellWorldPosition(cell.Pos);
                                //FDebug.DrawBounds3D(myBounds, Color.red, 1f);
                            }
                        }
