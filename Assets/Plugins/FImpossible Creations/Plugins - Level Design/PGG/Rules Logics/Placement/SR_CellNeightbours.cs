...
namespace FIMSpace.Generating.Rules.Placement
{
    public partial class SR_CellNeightbours : SpawnRuleBase, ISpawnProcedureType
    {
        ...
        private bool CheckNeightbourCellAllow(FieldCell cell, SpawnData spawn, Vector3 rotationOffset, FGenGraph<FieldCell, FGenPoint> grid, Vector3? restrictDirection = null)
        {
            ...
            CheckNeightbourCellsCorrectness(toCheck, ref allCorrect, ref correctCount);

            return allCorrect;
        }

        protected virtual bool CheckNeightbourBaseCondition(FieldCell toCheck)
        {
            return SpawnRules.CheckNeightbourCellAllow(CheckedCellsMustBe, toCheck, occupiedByTag, CheckMode);
        }

        protected virtual void CheckNeightbourCellsCorrectness(List<FieldCell> toCheck, ref bool allCorrect, ref int correctCount)
        {
            for (int i = 0; i < toCheck.Count; i++)
            {
                if (CheckNeightbourBaseCondition(toCheck[i]))
                {
