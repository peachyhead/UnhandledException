...
namespace FIMSpace.Graph
{
    public class PlannerGraphWindow : EditorWindow
    {
        ...
        public static void Init(FieldPlanner parent, IPlanNodesHandler graph, string title)
        {
            PlannerGraphWindow alreadyWindow = (PlannerGraphWindow)GetWindow(typeof(PlannerGraphWindow));
            PlannerGraphWindow window = alreadyWindow;
            ...
            window.titleContent = new GUIContent("Graph", Resources.Load<Texture2D>("SPR_PlanSmall"), "Build Planner - Graph Window");
            window.Show();
            ...
            window.graphView = FieldPlanner.EViewGraph.Procedures_CustomGraphs;
            window.CustomGraphName = title;
            window.currentCustomGraph = graph;
            window.currentContainer = graph.NodesHandler_Container;
            window.currentSetup = parent;
            window.currentFunction = null;
        }
        ...
        private void OnGUI()
        {
            ...
            if (BuildPlannerGraphDraw != null)
            {
                ...
                else if (BuildPlannerGraphDraw.DisplayMode == FieldPlanner.EViewGraph.Procedures_Placement)
                    BuildPlannerGraphDraw.Tex_Net = Tex_Net;
                else if (BuildPlannerGraphDraw.DisplayMode == FieldPlanner.EViewGraph.Procedures_CustomGraphs)
                {
                    if (currentCustomGraph != null)
                    {
                        if (Tex_Net3 != null) BuildPlannerGraphDraw.Tex_Net = Tex_Net3;
                        BuildPlannerGraphDraw.CustomGraphName = CustomGraphName;
                        BuildPlannerGraphDraw.SetCustomGraph(currentCustomGraph);
                    }
                }
