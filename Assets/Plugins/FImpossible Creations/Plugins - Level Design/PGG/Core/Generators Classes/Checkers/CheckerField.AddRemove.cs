...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField
    {
        ...
        public void Join(CheckerField other)
        {
        ...
        public void InjectToGrid(FGenGraph<FieldCell, FGenPoint> mainCorridorsGrid)
        {
            for (int i = 0; i < ChildPos.AllApprovedCells.Count; i++)
                mainCorridorsGrid.AddCell(GridPos(i));
        }

        public void AddPositions(List<Vector2Int> nRect, bool local = false)
        {
            if (local)
