...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        {
            return MatrixInverse.MultiplyPoint3x4(pos);
        }
        ...
        public bool IsCellAligning(CheckerField3D otherField, FieldCell otherFieldCell)
        {
            if (otherField == null) return FailedToSet;
            if (otherFieldCell == null) return FailedToSet;

            if (ContainsWorld(otherField.LocalToWorld(otherFieldCell.Pos + new Vector3Int(1, 0, 0))) == true) return true;
            if (ContainsWorld(otherField.LocalToWorld(otherFieldCell.Pos + new Vector3Int(-1, 0, 0))) == true) return true;
            if (ContainsWorld(otherField.LocalToWorld(otherFieldCell.Pos + new Vector3Int(0, 0, 1))) == true) return true;
            if (ContainsWorld(otherField.LocalToWorld(otherFieldCell.Pos + new Vector3Int(0, 0, -1))) == true) return true;
        ...
        public List<Vector3Int> GenerateLinesTowards(Vector3Int start, Vector3Int end, float nonDiagonal = 0.3f, int thickness = 1, List<LineFindHelper> directions = null, LineFindHelper.ERadiusType radiusType = LineFindHelper.ERadiusType.RectangleRadius, bool yRadius = false, bool clearOverpaint = false, bool eraseFinishCell = false)
        {
            ...
            while (currentPos != end)
            {
                ...
                for (int i = 0; i < directions.Count; i++)
                {
                    ...
                    float stepCost = (targetPos - end).magnitude + directions[i].Cost; // initial distance towards target from new position

                    if (nonDiagonal > 0f) if (fIters == 0) { if (directions[i].Dir.magnitude >= 1f) stepCost += nonDiagonal; }

                    if (directions[i].Dir != lastDir)
                    {
                        stepCost += nonDiagonal;
                    }
            ...
            if (thickness > 1)
            {
                ...
                if (clearOverpaint)
                {
                    ...
                    for (int b = 1 - (eraseFinishCell ? 1 : 0); b <= thickness; b++)
                    {
                        for (int s = -thickness; s <= thickness; s++)
                        {
                            Vector3Int pos = back * b;
        ...
        public class LineFindHelper
        {
            ...
            /// <summary> Allow to use this direction again only after few steps facing towards the same direction </summary>
            public int DirContinuityRequirement;
            /// <summary> Allow to use this direction only when few steps before was facing towards the same direction </summary>
            public int DirOriginationRequirement;

            public bool DisallowFinishOn;
            public int AllowUseSinceStep;

            public float ChangeDirectionCost = 0f;
            public float KeepDirectionCost = 0f;
            ...
            public LineFindHelper(Vector3Int dir, float val)
            {
                ...
                AllowUseSinceStep = 0;
                DirContinuityRequirement = 0;
                DirOriginationRequirement = 0;
            ...
            public LineFindHelper(Vector3Int dir, float val, int forceAfter, bool disallowFinishWithIt = false, int allowUseSinceStep = 0, int dirContinuityReq = 0, int dirOriginationReq = 0)
            {
                Dir = dir;
                Cost = val;

                ForceChangeDirAfter = forceAfter;
                DisallowFinishOn = disallowFinishWithIt;
                AllowUseSinceStep = allowUseSinceStep;
                DirContinuityRequirement = dirContinuityReq;
                DirOriginationRequirement = dirOriginationReq;

                _Find_WasUsed = 0;
                _Find_Steps = 0;
            }

            public int _Find_WasUsed;
            public int _Find_Steps;
        ...
        private static void RefreshDefaultDirections()
        {
            if (defaultLineFindDirections.Count == 0)
            {
                ...
                defaultLineFindDirections3D.Add(new LineFindHelper(new Vector3Int(0, 1, 0), 1.2f, 0, true, 2, 3));
                defaultLineFindDirections3D.Add(new LineFindHelper(new Vector3Int(0, -1, 0), 1.2f, 0, true, 2, 3));
