...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        {
            Vector3Int pos = MatrixInverse.MultiplyPoint3x4(worldPos).V3toV3Int(); // RootPosition.V3toV3Int();
            return AddLocal(pos);
        ...
        {
            Vector3 locPos = WorldToLocal(pos);
            ...
            var cell = Grid.GetCell(locPos.V3toV3Int(), false);
        ...
        /// <summary> Fast check, no rotation support </summary>
        public bool ContainsWorldBasic(Vector3 pos)
        {
        ...
        {
            //FDebug.DrawBounds2D(new Bounds(pos, RootScale * 0.5f), Color.cyan, 0.1f);

            Vector3 locPos = myInvMatrix.MultiplyPoint3x4(pos);
            Vector3Int origPos = locPos.V3toV3Int();

            var cell = Grid.GetCell(origPos, false );
            //DebugLogDrawCellInWorldSpace(origPos, Color.yellow, 0.2f);
