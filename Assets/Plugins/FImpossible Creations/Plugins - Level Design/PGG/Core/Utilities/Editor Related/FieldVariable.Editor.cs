...
namespace FIMSpace.Generating
{
    public partial class FieldVariable
    {
        ...
        public static object Editor_DrawTweakableVariable(FieldVariable toDraw, object helper = null)
        {
            ...
            else if (v.ValueType == FieldVariable.EVarType.Color)
            {
                EditorGUIUtility.labelWidth = 70;
                v.SetValue(EditorGUILayout.ColorField("Color:", v.GetColor()));
            }
            else if (v.ValueType == FieldVariable.EVarType.Curve)
            {
                EditorGUIUtility.labelWidth = 70;

                if (v.helper.y != 0f)
                {
                    var fixedRect = new Rect(v.helper.x, 0f, v.helper.y - v.helper.x, 1f - 0f);
                    v.SetValue(EditorGUILayout.CurveField(v.GetCurve(), Color.cyan, fixedRect));
                }
                else
                    v.SetValue(EditorGUILayout.CurveField(v.GetCurve()));
            }
