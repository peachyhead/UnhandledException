                    ...
                    if (ParentPlanner.ParentBuildPlanner)
                    {
                        ...
                        do
                        {
                            ...
                            if ( _EditorDebugMode)
                            {
                                ...
                                {
                                    if (planner.LatestChecker.IsCollidingWith(all[a].LatestChecker))
                                    FDebug.DrawBounds3D(all[a].LatestChecker.GetFullBoundsWorldSpace(), Color.red);
                                }
                            }

                        }
                        while (planner.LatestChecker.IsCollidingWith(all)  && pushes < 32);
                    }
            }
            else
            {
                ...
                while (planner.LatestChecker.IsCollidingWith(checkCollWith) && pushes < 32);
            }
