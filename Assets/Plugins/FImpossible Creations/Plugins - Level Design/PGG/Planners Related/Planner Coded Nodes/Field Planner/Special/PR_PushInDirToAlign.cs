...
namespace FIMSpace.Generating.Planning.PlannerNodes.Field.Special
{
    ...
    public class PR_PushInDirToAlign : PlannerRuleBase
    {
        public override string GetDisplayName(float maxWidth = 120) { return wasCreated ? "Push For Align" : "Push In Dir For Align"; }
        public override string GetNodeTooltipDescription { get { return "If there is detected possibility to align in provided direction, algorithm will try finding alignment position there."; } }
        public override EPlannerNodeType NodeType { get { return EPlannerNodeType.WholeFieldPlacement; } }
        public override Vector2 NodeSize { get { return new Vector2(_EditorFoldout ? 270 : 222, _EditorFoldout ? 124 : 112); } }
            ...
            if (collideWithAll)
            {
                if (ParentPlanner)
                    if (ParentPlanner.ParentBuildPlanner)
                    {
                        ...
                        planner.LatestChecker.StepPushToAlignCollision(all, pushDir, 256);
                    }
            }
            else
            {
                List<FieldPlanner> checkCollWith = GetPlannersFromPort(CollisionWith, false);
                planner.LatestChecker.StepPushToAlignCollision(checkCollWith, pushDir, 256);
            }
