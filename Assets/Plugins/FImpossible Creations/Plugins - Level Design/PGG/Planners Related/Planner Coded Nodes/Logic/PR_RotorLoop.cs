...
namespace FIMSpace.Generating.Planning.PlannerNodes.Cells.Loops
{
    ...
    public class PR_RotorLoop : PlannerRuleBase
    {
        ...
        public override Vector2 NodeSize { get { return new Vector2(_EditorFoldout ? 220 : 188, _EditorFoldout ? 181 : 131); } }
        ...
        [NonSerialized] List<int> order = new List<int>(); // local since static was causing errors
        //[NonSerialized] List<int> order = new List<int>(); // local since static was causing errors
        //static List<int> orderPool = new List<int>();
        ...
        public override void Execute(PlanGenerationPrint print, PlannerResult newResult)
        {
            ...
            if (RandomizeOrder)
            {
                for (int r = 0; r < Rotations.Count; r++) order.Add(r);
                order.Shuffle();

                //while (orderPool.Count > 0)
                //{
                //    int nR = FGenerators.GetRandom(0, orderPool.Count);
                //    order.Add(orderPool[nR]);
                //    orderPool.RemoveAt(nR);
                //}
            }
            else
            {
            ...
            for (int r = 0; r < order.Count; r++)
            {
                ...
                if (Stop.GetInputValue == true)
                {
                    break;
                }

                IterationRotation.Value = Rotations[order[r]];
        ...
        public override void Editor_OnNodeBodyGUI(ScriptableObject setup)
        {
            ...
            if (_EditorFoldout)
            {
                ...
                if (sp == null) sp = baseSerializedObject.FindProperty("RandomizeOrder");
