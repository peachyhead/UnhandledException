...
namespace FIMSpace.Generating.Planning.PlannerNodes.Cells.Actions
{
    ...
    public class PR_GetCell : PlannerRuleBase
    {
        ...
        public override void OnStartReadingNode()
        {
            ...
            if (pos is FieldCell)
                cell = pos as FieldCell;
            if (pos is PGGCellPort.Data)
            {
                PGGCellPort.Data dta = (PGGCellPort.Data)pos;
                if (FGenerators.NotNull(dta.CellRef))
                    if (FGenerators.NotNull(dta.ParentChecker))
                        cell = checker.GetCellInWorldPos(dta.ParentChecker.LocalToWorld(dta.CellRef.Pos));
            }
            else
            {
                if (pos is Vector3) cell = checker.GetCellInWorldPos((Vector3)pos);
            }
