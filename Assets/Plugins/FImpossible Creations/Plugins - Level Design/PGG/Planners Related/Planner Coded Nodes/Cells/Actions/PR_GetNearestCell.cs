...
namespace FIMSpace.Generating.Planning.PlannerNodes.Cells.Actions
{
    ...
    public class PR_GetNearestCell : PlannerRuleBase
    {
        ...
        public override void OnStartReadingNode()
        {
            ...
            if (TryGetCentered)
            {
                bool wasAligning = chA.IsCellAligning(chB, nearestB);

                FieldCell centeredB =
                 chA.GetMostCenteredCellInAxis(chB, nearestA, nearestB, new Vector3Int(0, 0, 1));

                bool checkX = false;
                if (wasAligning && !chA.IsCellAligning(chB, centeredB)) checkX = true;
            ...
            NearestBCell.ProvideFullCellData(nearestB, chB, fieldB == null ? null : fieldB.LatestResult);
        ...
        {
            A.Editor_DisplayVariableName = true;
            B.Editor_DisplayVariableName = true;
