...
namespace FIMSpace.Graph
{
    ...
    public class PGGCellPort : NodePortBase
    {
        ...
        public struct Data : ICheckerReference
        {
            [NonSerialized] public FieldCell CellRef;
            [NonSerialized] public CheckerField3D ParentChecker;
            public CheckerField3D CheckerReference { get { if (ParentChecker == null) if (ParentResult != null) return ParentResult.Checker; return ParentChecker; } }

            [NonSerialized] public PlannerResult ParentResult;
            public FieldPlanner AcquirePlanner { get { if (ParentResult == null) return null; return ParentResult.ParentFieldPlanner; } }
        ...
        /// <summary> Returning checkerField with single cell inside </summary>
        public CheckerField3D GetSelectiveCellChecker()
        {
            if (IsNotConnected) return null;

            var mainChe = Checker;
            if (mainChe == null) return null;
            if (Cell == null) return null;

            CheckerField3D nChe = new CheckerField3D();
            nChe.CopyParamsFrom(mainChe);
            nChe.AddLocal(Cell.Pos);
            ...
            if (first != null)
            {
                ...
                if (tPort != null)
                {
                    if (tPort is PGGPlannerPort)
                    {
                        ...
                        var shpe = plPrt.GetInputCheckerSafe;
                        if (shpe != null)
                        {
        ...
        public Data? GetDataFromPort(IFGraphPort conn)
        {
            ...
            if (conn is PGGPlannerPort)
            {
                ...
                if (prt.IsContaining_Checker)
                {
                    Data nData = new Data();
                    nData.ParentChecker = prt.GetInputCheckerSafe;
                    return nData;
                }
            }
        ...
        /// <summary>
        /// First tries to get planner out of result reference, if can't then it tries to get planner reference out of optional field reference placed in checker
        /// </summary>
        public FieldPlanner GetInputPlannerIfPossible
        {
            get
            {
                if (PortState() == EPortPinState.Empty) return null;
                ...
                if (d.Value.AcquirePlanner) return d.Value.AcquirePlanner;

                if (d.Value.ParentChecker != null)
                {
                    //if (d.Value.ParentChecker.CellsFieldParent) return d.Value.ParentChecker.CellsFieldParent;
                    if (d.Value.ParentChecker.SubFieldPlannerReference) return d.Value.ParentChecker.SubFieldPlannerReference;
                }
        ...
        {
            object val = base.GetPortValueCall(onReadPortCall);
        ...
        public override bool OnClicked(Event e)
        {
            ...
            if (FGenerators.CheckIfExist_NOTNULL(cl))
            {
                ...
                {
                    GetInputCheckerValue.DebugLogDrawCellInWorldSpace(cl, Color.green * 0.8f, 0.15f);
#if UNITY_EDITOR
