...
namespace FIMSpace.Graph
{
    public abstract partial class FGraphDrawerBase
    {
        ...
        Rect TransposePortRect(FGraph_NodeBase node, IFGraphPort port)
        {
            ...
            {
                CollapseRectsUpdate(node, portB);
            ...
            if (node._EditorCollapse ) CollapseRectsUpdate(node, portB);

            return portB._E_LatestCorrectPortRect;
        }

        void CollapseRectsUpdate(FGraph_NodeBase node, NodePortBase portB)
        {
            portB._E_LatestCorrectPortRect.y = node._E_LatestRect.position.y + 10;

            float tgtPortX = node._E_LatestRect.position.x + node.CompressedNodeSizeX - 36;
            if (portB._E_LatestCorrectPortRect.x > tgtPortX) portB._E_LatestCorrectPortRect.x = tgtPortX;
        }
