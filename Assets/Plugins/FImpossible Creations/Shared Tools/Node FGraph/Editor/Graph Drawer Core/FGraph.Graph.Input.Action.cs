...
namespace FIMSpace.Graph
{
    ...
    public abstract partial class FGraphDrawerBase
    {
        ...
        protected virtual void Input_Paste(Event e)
        {
            ...
            for (int i = 0; i < _RememberedNodesRefs.Count; i++)
            {
                ProcessPasteOf(originPos, mostTopL, _RememberedNodesRefs[i]);
            }
        ...
        protected virtual FGraph_NodeBase ProcessPasteOf(Vector2 originPos, Vector2 mostTopL, FGraph_NodeBase node)
        {
            var inst = ScriptableObject.Instantiate(node);
