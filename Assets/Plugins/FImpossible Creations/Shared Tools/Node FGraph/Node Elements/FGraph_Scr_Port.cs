...
{
    public static class NodePortBaseUtilities
    {
        public static NodePortBase ToNodePortBase(this IFGraphPort port)
        {
            return port as NodePortBase;
        }
    }
    ...
    public class NodePortBase : IFGraphPort
    {
        ...
        {
            Default, Ghosting, Curled, Line, Hidden
        }
        ...
        public virtual void TriggerReadPort(bool callRead = false)
        {
            for (int i = 0; i < Connections.Count; i++)
            {
                ...
                if (port != null)
                {
                    ...
                    if (port.IsOutput) // Other port is output?
                    {
                        if (IsInput) port.ParentNode.DONT_USE_IT_YET_OnReadPort(port); // This port is input?
                        port.ParentNode.OnStartReadingNode();
                    }
                }
            }
        ...
        public string GetBaseConnectionPortName()
        {
            if (BaseConnection != null)
            {
                NodePortBase port = BaseConnection.PortReference as NodePortBase;
                if (port != null) return port.DisplayName;
            }
        ...
        //bool _editorAllowDrag
        //{
        //    get
        //    {
        //        if (AllowDragWire == false) return false;
        //        if (ParentNode != null) if (ParentNode._EditorCollapse) return false;
        //        return true;
        //    }
        //}
        ...
        public virtual void RefreshPortConnections<T>(List<T> allNodes) where T : FGraph_NodeBase
        {
            //bool found = false;
        ...
        public virtual bool OnClicked(Event e)
        {
            ...
            if (e.button == 1)
            {
                ...
                if (oPort != null)
                {
                    ...
                    menu.AddItem(new GUIContent("Line"), oPort.ConnectionDisplay == EPortConnectionDisplayMode.Line, () => { oPort.ConnectionDisplay = EPortConnectionDisplayMode.Line; });
